...
POSITIONS = [
    ...
    {"code": "OR",    "name": "L'Oréal",            "yahoo": "OR.PA",    "qty": 5,   "pru": 378.146,  "fallback": 382.05,
     "sector": "Beauté", "action": "Renforcer ~375 €", "tone": "buy",
...
def _from_closes(closes, price=None):
    """Cours et variation vs veille à partir d'une série de clôtures journalières."""
...
def fetch_price(symbol):
    """Retourne (cours, variation_jour_%, journal des tentatives)."""
    log = []

    # 1) API chart de Yahoo, appelée directement
    for host in ("query1", "query2"):
        try:
            ...
            price, day = _from_closes(closes, res["meta"].get("regularMarketPrice"))
            if price:
                return price, day, log
    ...
    try:
        ...
        price, day = _from_closes(hist["Close"].dropna().tolist())
        if price:
            return price, day, log
...
def build_payload():
    ...
    for p, (price, day, log) in zip(POSITIONS, results):
        if price is None:
            failed[p["yahoo"]] = log
...
def api_debug():
    ...
    out = {}
    for p in POSITIONS:
        price, day, log = fetch_price(p["yahoo"])
        out[p["yahoo"]] = {"price": price, "day_pct": day, "tentatives_echouees": log}
    return jsonify(out)
...
<div class="wrap">
  ...
  <section>
    ...
    <div class="card tablewrap">
      <table>
        <thead><tr>
          <th>Valeur</th><th>Cours export</th><th>Cours live</th><th>Qté</th><th>PRU</th>
...
function render(d){
  ...
  document.getElementById('kpis').innerHTML = `
    ...
    <div class="kpi"><div class="l">Produit PE500 en attente</div>
      ...
      <div class="s">Estimation · total avec liquidités ${eur(d.total_with_cash)}</div></div>
    <div class="kpi"><div class="l">Meilleure ligne</div>
      <div class="v pos">${pct(d.best_pct,0)}</div>
      <div class="s">${d.best}</div></div>
    <div class="kpi"><div class="l">Ligne la plus basse</div>
      <div class="v ${cls(d.worst_pct)}">${pct(d.worst_pct)}</div>
  ...
  document.getElementById('rows').innerHTML = d.rows.map(r=>`<tr>
      ...
      <td class="xp">${num(r.export_price)}\u00a0€</td>
      <td>${r.live?`<span class="px live"><i class="ldot"></i>${num(r.live_price)}\u00a0€</span>`:'<span class="est">indisponible</span>'}</td>
      <td>${r.qty}</td>
      <td>${num(r.pru)}\u00a0€</td>
