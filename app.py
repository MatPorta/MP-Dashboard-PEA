...
Version 2 — remplace la v1 Finnhub (ne retournait pas de données)
"""
...
POSITIONS = [
    ...
    {"nom": "L'Oréal",        "ticker": "OR.PA",   "qte": 5,   "pru": 378.146,  "strategie": "Renforcer",         "fallback": 386.65, "secteur": "Beauté / Consommation"},
...
MOUVEMENTS_PLANIFIES = [
    ...
    {"valeur": "L'Oréal",  "action": "Renforcer",           "detail": "Avec produit Accor (~17 titres) · Zone idéale 385–395 € · PRU cible ~382 €"},
...
def fetch_price(ticker: str):
    """Récupère le dernier cours via Yahoo Finance, avec fallback query2 puis None."""
...
def build_portfolio():
    """Construit la liste des positions avec cours live ou fallback."""
    now = time.time()
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .strat { font-size: 0.78rem; color: var(--accent); }
  ...
  .allocation-bar div { display: flex; align-items: center; justify-content: center; font-size: 0.68rem; color: #0b0e14; font-weight: 600; }
  ...
  <div class="panel">
    ...
    <div class="allocation-bar">
      ...
      <div style="width: {{ r.poids }}%; background: hsl({{ loop.index0 * 37 % 360 }}, 55%, 50%);" title="{{ r.nom }} — {{ '%.1f'|format(r.poids) }}%">
        {{ r.nom.split(' ')[0] if r.poids > 5 else '' }}
      </div>
      {% endfor %}
    </div>
    ...
    <div class="allocation-bar" style="margin-top: 18px;">
      ...
      <div style="width: {{ s.poids }}%; background: hsl({{ loop.index0 * 53 % 360 }}, 45%, 45%);" title="{{ s.nom }} — {{ '%.1f'|format(s.poids) }}%">
        {{ s.nom.split(' ')[0] if s.poids > 5 else '' }}
      </div>
      {% endfor %}
    </div>
