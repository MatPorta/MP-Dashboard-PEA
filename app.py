...
def _from_series(series, price=None, today=None):
    ...
    # couvre au moins un jour ouvré avant aujourd'hui.
    prev = next((c for d, c in reversed(series) if d < today), None)
...
def fetch_price(symbol):
    ...
    for host in ("query1", "query2"):
        try:
            ...
            price, day, ytd_ref = _from_series(series, meta.get("regularMarketPrice"), today_local)
            prev = next((c for d, c in reversed(series) if today_local and d < today_local), None)
            recent = [f"{d.isoformat()}={c}" for d, c in series[-5:]]
            log.append(f"{host}: price={price} prev={prev} day={day} today_local={today_local} "
                       f"gmtoffset={gmtoffset} 5 dernières barres={recent}")
            if price:
                return price, day, ytd_ref, log
    ...
    try:
        ...
        price, day, ytd_ref = _from_series(series)
        if price:
            return price, day, ytd_ref, log
...
def api_debug():
    ...
    out = {}
    for p in POSITIONS:
        price, day, ytd_ref, log = fetch_price(p["yahoo"])
        out[p["yahoo"]] = {"price": price, "day_pct": day, "ytd_ref": ytd_ref, "journal": log}
    return jsonify(out)
