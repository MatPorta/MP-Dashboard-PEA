...
def _from_series(series, price=None, prev_close=None):
    ...
    # La clôture de la veille doit venir de meta.previousClose / chartPreviousClose quand
    # elle est disponible : on ne peut pas fiablement la déduire de closes[-2], car selon le
    # moment de l'appel, Yahoo a parfois déjà ajouté la barre du jour à "series" (closes[-2] =
    # hier, correct) et parfois pas encore (closes[-2] = avant-hier, ce qui calcule une
    # variation sur 2 séances au lieu d'une, sans que rien ne le signale).
    if prev_close:
        prev = float(prev_close)
    elif len(closes) >= 2 and closes[-1] != price:
        # Barre du jour absente de la série : la dernière clôture connue est bien hier.
        prev = closes[-1]
    else:
