...
POSITIONS = [
    ...
    {"code": "OR",    "name": "L'Oréal",            "yahoo": "OR.PA",    "qty": 5,   "pru": 378.146,  "fallback": 382.05,
     "sector": "Beauté", "action": "Renforcer ~375 €", "tone": "buy", "new_2026": True,
     "note": "Produit PE500, puis produit Accor"},
...
MOVES = [
    ...
    {"code": "OR", "kind": "buy", "verb": "Renforcer", "target": 375.0, "budget": CASH_PENDING,
     "text": "Avec le produit du PE500 (~4 155 €). Le produit d'Accor viendra compléter."},
...
# Agenda : uniquement les valeurs du portefeuille
AGENDA = [
    ...
    {"date": "2026-11-15", "label": "Automne", "code": "OR", "title": "L'Oréal — chiffres T3",
     "text": "Premières contributions de Gucci Beauté."},
...
def _from_series(series, price=None, today=None):
    """series : liste de (date, clôture) triée par date, dates dans le fuseau de la bourse.
    today : date locale considérée comme "aujourd'hui" (si None, on prend la dernière date
    connue de la série). Retourne (cours, variation vs veille en %, clôture du 31/12 précédent,
    date de la clôture de référence utilisée pour la variation — utile pour signaler un trou
    de données : ex. Yahoo ne publie pas toujours la clôture des ETF peu suivis le jour même)."""
    series = [(d, float(c)) for d, c in series if c is not None]
    if not series:
        return None, None, None, None
    price = float(price) if price else series[-1][1]
    if today is None:
        today = series[-1][0]
    # Clôture de la veille = dernière clôture dont la date est STRICTEMENT antérieure à
    # aujourd'hui. Contrairement à closes[-2] (fragile : dépend de si la barre du jour est
    # déjà dans la série) ou à meta.previousClose/chartPreviousClose (absents ou mal définis
    # selon les titres), cette comparaison de dates est toujours correcte tant que la série
    # couvre au moins un jour ouvré avant aujourd'hui. Si la barre la plus récente avant
    # aujourd'hui a plus d'1 jour d'écart (ex. Yahoo n'a pas encore publié la clôture de la
    # veille pour ce titre), la variation porte alors sur plusieurs séances, pas une seule.
    prev_date, prev = next(((d, c) for d, c in reversed(series) if d < today), (None, None))
    day = (price / prev - 1) * 100 if prev else None
    ref_date = datetime(datetime.now().year - 1, 12, 31).date()
    ytd_ref = next((c for d, c in reversed(series) if d <= ref_date), None)
    return price, day, ytd_ref, prev_date


def fetch_price(symbol):
    """Retourne (cours, variation_jour_%, clôture 31/12 année précédente, écart en jours de la
    clôture de référence, journal des tentatives)."""
    log = []
    year = datetime.now().year
    p1 = int(datetime(year - 1, 12, 15, tzinfo=timezone.utc).timestamp())
    p2 = int(time.time()) + 86400

    # 1) API chart de Yahoo, appelée directement (depuis mi-décembre pour avoir la clôture du 31/12)
    for host in ("query1", "query2"):
        try:
            ...
            # On décale les timestamps UTC de l'offset de la bourse avant d'en tirer une date,
            # pour que "date locale" corresponde bien au jour de bourse (Paris/Amsterdam),
            # et non au jour UTC qui peut diverger en toute fin/début de séance.
            series = [(datetime.fromtimestamp(t + gmtoffset, timezone.utc).date(), c)
                      for t, c in zip(stamps, closes)]
            market_time = meta.get("regularMarketTime")
            today_local = (datetime.fromtimestamp(market_time + gmtoffset, timezone.utc).date()
                           if market_time else None)
            price, day, ytd_ref, prev_date = _from_series(series, meta.get("regularMarketPrice"), today_local)
            gap = (today_local - prev_date).days if (today_local and prev_date) else None
            recent = [f"{d.isoformat()}={c}" for d, c in series[-5:]]
            log.append(f"{host}: price={price} prev_date={prev_date} gap={gap}j day={day} "
                       f"today_local={today_local} gmtoffset={gmtoffset} 5 dernières barres={recent}")
            if price:
                return price, day, ytd_ref, gap, log
            log.append(f"{host}: réponse sans cours")
        except Exception as e:
            log.append(f"{host}: {type(e).__name__}")

    # 2) Repli : bibliothèque yfinance (gère cookies et navigation type navigateur)
    try:
        import yfinance as yf
        hist = yf.Ticker(symbol).history(start=f"{year - 1}-12-15", interval="1d")
        series = [(d.date(), c) for d, c in hist["Close"].dropna().items()]
        price, day, ytd_ref, prev_date = _from_series(series)
        gap = (series[-1][0] - prev_date).days if (series and prev_date) else None
        if price:
            return price, day, ytd_ref, gap, log
        log.append("yfinance: aucune donnée")
    except ImportError:
...
def build_moves(rows):
    ...
    for m in MOVES:
        ...
        item["name"] = r["name"] if r else m.get("name", m["code"])
        if r:
            item["price"] = r["price"]
...
def build_payload():
    ...
    for p, (price, day, ytd_ref, gap, log) in zip(POSITIONS, results):
        if price is None:
            failed[p["yahoo"]] = log
    ...
    return {
        ...
        "agenda": [dict(a, color=next(r["color"] for r in rows if r["code"] == a["code"])) for a in AGENDA],
...
def api_debug():
    ...
    out = {}
    for p in POSITIONS:
        price, day, ytd_ref, gap, log = fetch_price(p["yahoo"])
        out[p["yahoo"]] = {"price": price, "day_pct": day, "day_gap": gap, "ytd_ref": ytd_ref, "journal": log}
    return jsonify(out)
...
function render(d){
  ...
  document.getElementById('rows').innerHTML = d.rows.map(r=>`<tr>
      ...
      <td>${r.day==null?'<span class="est">—</span>':`<span class="dpill ${cls(r.day)}">${pct(r.day,2)}</span>${r.day_gap>1?`<span class="est" title="Clôture de référence non disponible pour hier chez Yahoo : variation sur ${r.day_gap} jours">J-${r.day_gap}</span>`:''}`}</td>
      <td>${r.qty}</td>
      <td>${num(r.pru)}\u00a0€</td>
