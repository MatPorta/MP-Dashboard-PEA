...
def _from_series(series, price=None, prev_close=None):
    """series : liste de (date, clôture) triée par date.
    Retourne (cours, variation vs veille en %, dernière clôture de l'année précédente)."""
    series = [(d, float(c)) for d, c in series if c is not None]
    if not series:
        return None, None, None
    closes = [c for _, c in series]
    price = float(price) if price else closes[-1]
    # La clôture de la veille doit venir de meta.previousClose quand elle est disponible :
    # La variation jour ne repose que sur meta.previousClose (la vraie clôture de la veille,
    # celle que Yahoo utilise lui-même pour sa propre variation affichée). Si Yahoo ne le
    # fournit pas pour ce titre, on ne devine pas : la variation jour reste indisponible
    # plutôt que d'afficher un chiffre potentiellement faux (voir historique : closes[-2]
    # et chartPreviousClose se sont tous deux révélés peu fiables ici).
...
def fetch_price(symbol):
    ...
    for host in ("query1", "query2"):
        try:
            ...
            series = [(datetime.fromtimestamp(t, timezone.utc).date(), c) for t, c in zip(stamps, closes)]
            meta = res.get("meta", {})
            prev_close = meta.get("previousClose")
            price, day, ytd_ref = _from_series(series, meta.get("regularMarketPrice"), prev_close)
            if price:
    ...
    try:
        ...
        prev_close = None
        try:
            prev_close = yf.Ticker(symbol).fast_info.get("previous_close")
        except Exception:
            pass
        price, day, ytd_ref = _from_series(series, prev_close=prev_close)
        if price:
            return price, day, ytd_ref, log
