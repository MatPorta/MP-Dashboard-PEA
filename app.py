...
<style>
  ...
  *{box-sizing:border-box}
  ...
  table{width:100%;border-collapse:separate;border-spacing:0;min-width:900px;font-variant-numeric:tabular-nums}
  ...
  .px{font-weight:600}
  .chg{font-size:12px;display:block;font-weight:500;margin-top:1px}
...
<div class="wrap">
  ...
  <section>
    ...
    <div class="card tablewrap">
      <table>
        <thead><tr>
          <th>Valeur</th><th>Cours</th><th>Qté</th><th>PRU</th>
...
function render(d){
  ...
  document.getElementById('rows').innerHTML = d.rows.map(r=>`<tr>
      ...
      <td><span class="px">${num(r.price)}\u00a0€</span>${r.live?'':'<span class="est">export</span>'}
        <span class="chg ${cls(r.day||0)}">${r.day==null?'':pct(r.day,2)}</span></td>
      <td>${r.qty}</td>
      <td>${num(r.pru)}\u00a0€</td>
